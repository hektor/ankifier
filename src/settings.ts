...
export class SettingsTab extends PluginSettingTab {
  ...
  create_collapsible(name: string) {
    ...
    div.addEventListener('click', function () {
      ...
      const icon = this.firstElementChild.firstElementChild as HTMLElement
  ...
  setup_note_table() {
    ...
    const note_type_table = containerEl.createEl('table', {
  ...
  setup_syntax() {
    ...
    // eslint-disable-next-line
    const plugin = (this as any).plugin
    const syntax_settings = containerEl.createEl('h3', {
  ...
  setup_defaults() {
    ...
    // eslint-disable-next-line
    const plugin = (this as any).plugin
    const defaults_settings = containerEl.createEl('h3', {
      text: 'Defaults',
    })
  ...
  get_folders(): TFolder[] {
    ...
    for (const folder of folder_list) {
      const filtered_list: TFolder[] = folder.children.filter((element) =>
  ...
  setup_folder_table() {
    ...
    const folder_table = containerEl.createEl('table', {
      cls: 'anki-settings-table',
    })
    const head = folder_table.createTHead()
    const header_row = head.insertRow()
    for (const header of ['Folder', 'Folder Deck', 'Folder Tags']) {
      const th = document.createElement('th')
      th.appendChild(document.createTextNode(header))
      header_row.appendChild(th)
    }
